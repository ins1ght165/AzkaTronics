...
def iphone_view_or_compare(apple_specs):
    View_or_Compare="""Do you want to view the specs of {} or compare it with another device:
1) View 
2) Compare
""".format(iphone[apple_specs-1][0])

    for x in range(len(Phone_speclist)):
        view_specs_L.append([Phone_speclist[x],iphone[apple_specs-1][1][x]])
...
""".format(samsung_phones[samsung_specs-1][0])
    ...
    for x in range(len(Phone_speclist)):
        view_specs_L.append([Phone_speclist[x],samsung_phones[samsung_specs-1][1][x]])
    return View_or_Compare
...
""".format(huawei_phones[huawei_specs-1][0])
    ...
    for x in range(len(Phone_speclist)):
        view_specs_L.append([Phone_speclist[x],huawei_phones[huawei_specs-1][1][x]])
    return View_or_Compare
...
""".format(google_phones[google_specs-1][0])
    ...
    for x in range(len(Phone_speclist)):
        view_specs_L.append([Phone_speclist[x],google_phones[google_specs-1][1][x]])
    return View_or_Compare
...
""".format(Tablets_iphone[ipad_specs-1][0])
    ...
    for x in range(len(Phone_speclist)):
        view_specs_L.append([Phone_speclist[x],Tablets_iphone[ipad_specs-1][1][x]])
    return View_or_Compare
...
""".format(Tablets_samsung[samsung_tablet_specs-1][0])
    ...
    for x in range(len(Phone_speclist)):
        view_specs_L.append([Phone_speclist[x],Tablets_samsung[-1][1][x]])
    return View_or_Compare
...
""".format(laptop_lenovo[lenovo_laptop_specs-1][0])
    ...
    for x in range(len(Laptop_speclist)):
        view_specs_L.append([Laptop_speclist[x],laptop_lenovo[-1][1][x]])
    return View_or_Compare
...
""".format(laptop_HP[hp_laptop_specs-1][0])
    ...
    for x in range(len(Laptop_speclist)):
        view_specs_L.append([Laptop_speclist[x],laptop_HP[-1][1][x]])
    return View_or_Compare
...
""".format(pc_lenovo[lenovo_pc_specs-1][0])
    ...
    for x in range(len(Desktop_speclist)):
        view_specs_L.append([Desktop_speclist[x],pc_lenovo[-1][1][x]])
    return View_or_Compare
...
""".format(pc_hp[hp_pc_specs-1][0])
    ...
    for x in range(len(Desktop_speclist)):
        view_specs_L.append([Desktop_speclist[x],pc_hp[-1][1][x]])
    return View_or_Compare
def featured_items():
...
def second_phone_choice(msg):
    if len(prev_selections)==11:
        prev_selections.append("Second Manf listed for compare")
        return """Select a device manufacturer:
    1)	Apple.
    2)	Samsung.
    3)	Huawei.
    4)	Google.
    """
    if len(prev_selections)==13:
        if not validate_selection(1,4,msg):
            prev_selections.pop(-1)
            return "Please select a valid choice: "
    if len(prev_selections)>=13:
        if int(prev_selections[12])==1:
            if len(prev_selections)==13:
                compare_lists.append(iphone.copy())
                prev_selections.append("Iphones listed 2")
                txt="Which Apple phone would you like to view?\n"
                n=1
                for x in iphone:
                    txt+="{})   {}\n".format(n,x[0])
                    n+=1
                return txt
            if len(prev_selections)==15:
                if not validate_selection(1,8,msg):
                    prev_selections.pop(-1)
                    return "Please select a valid choice: "
                compare_lists.append(int(msg)-1)
                prev_selections.append("Compare Selected")
                return "Compare table will open in new window.\nPlease press enter. "
            
        if int(prev_selections[12])==2:
            if len(prev_selections)==13:
                compare_lists.append(samsung_phones.copy())
                prev_selections.append("Samsung phones listed 2")
                txt="Which Samsung phone would you like to view?\n"
                n=1
                ...
                return txt
            if len(prev_selections)==15:
                if not validate_selection(1,9,msg):
                    prev_selections.pop(-1)
                    return "Please select a valid choice: "
                compare_lists.append(int(msg)-1)
                prev_selections.append("Compare Selected")
        ...
        if int(prev_selections[12])==3:
            if len(prev_selections)==13:
                compare_lists.append(huawei_phones.copy())
                prev_selections.append("Huawei Phones listed 2")
                txt="Which Huawei phone would you like to view?\n"
                n=1
                ...
                return txt
            if len(prev_selections)==15:
                if not validate_selection(1,7,msg):
                    prev_selections.pop(-1)
                    return "Please select a valid choice: "
                compare_lists.append(int(msg)-1)
                prev_selections.append("Compare Selected")
        ...
        if int(prev_selections[12])==4:
            if len(prev_selections)==13:
                compare_lists.append(google_phones.copy())
                txt="Which Google phone would you like to view?\n"
                n=1
...
def second_tablet_choice(msg):
    if len(prev_selections)==11:
        prev_selections.append("Second Manf listed for compare")
        return """Here's a list of the tablet manufacturers: (Please choose an option)
    1) Apple
    2) Samsung
    """
    if len(prev_selections)==13:
    ...
    if len(prev_selections)>=13:
        if int(prev_selections[12])==1:
            if len(prev_selections)==13:
                compare_lists.append(Tablets_iphone.copy())
                prev_selections.append("iPads listed 2")
                txt="Which iPad would you like to view?\n"
                n=1
        ...
        if int(prev_selections[12])==2:
            if len(prev_selections)==13:
                compare_lists.append(Tablets_samsung.copy())
                prev_selections.append("Samsung Tablets listed 2")
                txt="Which Samsung Tablet would you like to view?\n"
                n=1
                ...
                return txt
            if len(prev_selections)==15:
                if not validate_selection(1,3,msg):
                    prev_selections.pop(-1)
                    return "Please select a valid choice: "
                compare_lists.append(int(msg)-1)
                prev_selections.append("Compare Selected")
...
def second_laptop_choice(msg):
    if len(prev_selections)==11:
        prev_selections.append("Second Manf listed for compare")
        return """Here's a list of the laptop manufacturers: (Please choose an option)
1) Lenovo
2) Hp
"""
    if len(prev_selections)==13:
    ...
    if len(prev_selections)>=13:
        if int(prev_selections[12])==1:
            if len(prev_selections)==13:
                compare_lists.append(laptop_lenovo.copy())
                prev_selections.append("Lenovo laptopslisted 2")
                txt="Which Lenovo laptop would you like to view?\n"
                n=1
        ...
        if int(prev_selections[12])==2:
            if len(prev_selections)==13:
                compare_lists.append(laptop_HP.copy())
                prev_selections.append("HP laptops listed 2")
                txt="Which Hp laptop would you like to view?\n"
                n=1
                for x in laptop_HP:
                    txt+="{})   {}\n".format(n,x[0])
                    n+=1
...
def second_desktop_choice(msg):
    if len(prev_selections)==11:
        prev_selections.append("Second Manf listed for compare")
        return """Here's a list of the PC manufacturers: (Please choose an option)
1) Lenovo
2) Hp
"""
    if len(prev_selections)==13:
    ...
    if len(prev_selections)>=13:
        if int(prev_selections[12])==1:
            if len(prev_selections)==13:
                compare_lists.append(pc_lenovo.copy())
                prev_selections.append("Lenovo Pcs listed 2")
                txt="Which Lenovo PC would you like to view?\n"
                n=1
                for x in pc_lenovo:
                    txt+="{})   {}\n".format(n,x[0])
                    n+=1
        ...
        if int(prev_selections[12])==2:
            if len(prev_selections)==13:
                compare_lists.append(pc_hp.copy())
                prev_selections.append("HP Pcs listed 2")
                txt="Which Hp PC would you like to view?\n"
                n=1
                for x in pc_hp:
                    txt+="{})   {}\n".format(n,x[0])
                    n+=1
...
def compare_devices(device_specs,manf1,phone1,manf2,phone2):
    view_specs_L.clear()
    view_specs_L.append(["Specifications",manf1[phone1][0],manf2[phone2][0]])
    for x in range(len(device_specs)):
        view_specs_L.append([device_specs[x],manf1[phone1][1][x],manf2[phone2][1][x]])
...
def sphone_apple(cust_manf, iphone_model, iphone_model1):
    if cust_manf == 1:
...
def email_validate(email):
    ...
    else: validation_message.append("The email format is incorrect. Please Re-enter your email: ")
    return valid
...
def validate_name(name):
    ...
    if not any(char.isdigit() or char in string.punctuation.replace("-","") for char in name):
            val=True
    else: validation_message.append("Name can't contain numbers or special characters (excluding '-').\nPlease re-enter your name: ")
    return val
...
def validate_phonenumber(msg):
    valid=False
    if any(char in list(string.punctuation) for char in msg) or any(char.isalpha() for char in msg) or len(msg)!=11:
        validation_message.append("The phone number format is either wrong or number lenght(11) is incorrect.")
    else:
        valid=True
    return valid
...
def validate_YorN(msg):
    if msg.upper()!="N" and msg.upper()!="Y":
        validation_message.append("Please select a valid option (Y/N): ")
        valid= False
    else: valid= True
    return valid
...
def validate_date(msg):
    ...
    valid = True
    try:
        valid = bool(datetime.strptime(msg, format))
    except ValueError:
        validation_message.append("The date format you enterer is incorrect.\nPlease enter the date in the format (DD/MM/YYYY): ")
        valid = False
    return valid
#register and login#################
def login_file():
...
def locations_function(msg):
    if len(prev_selections)==3:
            prev_selections.append("Locations listed")
            return """We have stores in 1.{}, 2.{}, 3.{}, 4.{}, 5.{}, 6.{}.
        """.format(locations_list[0], locations_list[1], locations_list[2], locations_list[3], locations_list[4], locations_list[5])
    if len(prev_selections)>=5:
            if len(prev_selections)==5 and (not validate_selection(1,6,msg)):
                prev_selections.pop(-1)
                return "Please enter a valid choice."
            prev_selections.append("Specific Location detailed")
            if int(prev_selections[4]) == 1: 
            # return order_pizza(input, prev_selections)
                return """Azka Tronics store in {} in the {} floor.
        Mall Location: {}.""".format(locations_list[0],"second","https://goo.gl/maps/fMR5h9nFsKjaDU1H9")
            elif int(prev_selections[4]) == 2: 
                # return locations(input, prev_selections)
                return """Azka Tronics store in {} in the {} floor.
        Mall Location: {}.""".format(locations_list[1],"fourth","https://goo.gl/maps/Ef2DpumrqXdtSqoTA")
            elif int(prev_selections[4]) == 3:
                # return prices(input, prev_selections)
                return """Azka Tronics store in {} in the {} floor.
        Mall Location: {}.""".format(locations_list[2],"third","https://goo.gl/maps/og5dbUqiaBkYcP3j9")
            elif int(prev_selections[4]) == 4:
                # return work_for_us(input, prev_selections)
                return """Azka Tronics store in {} in the {} floor.
        Mall Location: {}.""".format(locations_list[3],"ground","https://goo.gl/maps/WMf3mk7q98gJC5hEA")
            elif int(prev_selections[4]) == 5:
                # return work_for_us(input, prev_selections)
                return """Azka Tronics store in {} in the {} floor.
        Mall Location: {}.""".format(locations_list[4],"fifth","https://goo.gl/maps/5vMtoLoJL5YVpqBo7")
            elif int(prev_selections[4]) == 6:
                # return work_for_us(input, prev_selections)
                return """Azka Tronics store in {} in the {} floor.
...
def track_order(Order_date):
    ...
    days = datetime.now()
    #print("Today's date is: ",days.strftime('%A'), Today_date,"\n")
...
def m_chatbot(inputmsg,prev_selections):
    cust_need=0
    comparechoice=None

    if len(prev_selections)==1:
        prev_selections.append("Main categories listed")
        return """
...
"""
    elif len(prev_selections)==3:
        if not validate_selection(1,6,inputmsg):
            prev_selections.pop(-1)
            return "Please enter a valid choice."
    if len(prev_selections)>=3:
        if int(prev_selections[2]) == 1: 
            cust_need=1
        elif int(prev_selections[2]) == 2: 
            cust_need=2
        elif int(prev_selections[2]) == 3:
            cust_need=3
        elif int(prev_selections[2]) == 4:
            cust_need=4
        elif int(prev_selections[2]) == 5:
            cust_need = 5
    ...
    if cust_need==2:
        ...
        if len(prev_selections)==3:
            prev_selections.append("View specs listed")
            return """What device do you want to view the specs of:
    1)	Smartphone.
    2)	Tablet.
    3)	Laptop.
    4)	Desktop PC.   
    """
        if len(prev_selections)>=5:
            if len(prev_selections)==5 and (not validate_selection(1,4,inputmsg)):
                prev_selections.pop(-1)
                return "Please enter a valid choice."
            if int(prev_selections[4]) == 1: 
            # return order_pizza(input, prev_selections)
            ...
            if device_specs==1:
                if len(prev_selections)==5:
                    compare_lists.append(Phone_speclist.copy())
                    prev_selections.append("Smartphone manf listed")
                    return """Select a device manufacturer:
        1)	Apple.
        2)	Samsung.
        3)	Huawei.
        4)	Google.
        """
                if len(prev_selections)>=7:
                    if len(prev_selections)==7 and (not validate_selection(1,4,inputmsg)):
                        prev_selections.pop(-1)
                        return "Please enter a valid choice."
                    if int(prev_selections[6]) == 1: 
                    # return order_pizza(input, prev_selections)
                        smartphone_specs=1
                    elif int(prev_selections[6]) == 2: 
                        # return locations(input, prev_selections)
                        smartphone_specs=2
                    elif int(prev_selections[6]) == 3:
                        # return prices(input, prev_selections)
                        smartphone_specs=3
                    elif int(prev_selections[6]) == 4:
                        # return work_for_us(input, prev_selections)
                        smartphone_specs=4
                    ...
                    if smartphone_specs==1:
                        if len(prev_selections)==7:
                            ...
                            for phone in iphone:
                                txt+="{})   {}\n".format(n,phone[0])
                                n+=1
                            prev_selections.append("Iphones listed")
                            return txt
                        if len(prev_selections)>=9:
                            if len(prev_selections)==9 and (not validate_selection(1,8,inputmsg)):
                                prev_selections.pop(-1)
                                return "Please enter a valid choice."
                            if len(prev_selections)==9:
                                apple_specs=int(inputmsg)
                                compare_lists.append(apple_specs-1)
                                comparechoiceask=iphone_view_or_compare(apple_specs)
                                prev_selections.append("View or Compare?")
                                return comparechoiceask
                            if len(prev_selections)>=11:
                                if len(prev_selections)==11 and (not validate_selection(1,2,inputmsg)):
                                    prev_selections.pop(-1)
                                    return "Please enter a valid choice."
                                if int(prev_selections[10]) == 1 and len(prev_selections)==11:
                                    prev_selections.append("View selected")
                                    return "All the specifications for the selected device will open in a new window,\n Please press enter. "
                                elif int(prev_selections[10]) == 2:
                                    comparechoice=2
                    if smartphone_specs==2:
                        if len(prev_selections)==7:
                            ...
                            for x in samsung_phones:
                                txt+="{})   {}\n".format(n,x[0])
                                n+=1
                            prev_selections.append("Samsung phones listed")
                            return txt
                        if len(prev_selections)>=9:
                            if len(prev_selections)==9 and (not validate_selection(1,9,inputmsg)):
                                prev_selections.pop(-1)
                                return "Please enter a valid choice."
                            if len(prev_selections)==9:
                                samsung_specs=int(inputmsg)
                                compare_lists.append(samsung_specs-1)
                                comparechoiceask=samsung_view_or_compare(samsung_specs)
                                prev_selections.append("View or Compare?")
                                return comparechoiceask
                            if len(prev_selections)>=11:
                                if len(prev_selections)==11 and (not validate_selection(1,2,inputmsg)):
                    ...
                    if smartphone_specs==3:
                        if len(prev_selections)==7:
                            ...
                            for x in huawei_phones:
                                txt+="{})   {}\n".format(n,x[0])
                                n+=1
                            prev_selections.append("Huawei phones listed")
                            return txt
                        if len(prev_selections)>=9:
                            if len(prev_selections)==9 and (not validate_selection(1,7,inputmsg)):
                                prev_selections.pop(-1)
                                return "Please enter a valid choice."
                            if len(prev_selections)==9:
                                huawei_specs=int(inputmsg)
                                compare_lists.append(huawei_specs-1)
                                comparechoiceask=hauwei_view_or_compare(huawei_specs)
                                prev_selections.append("View or Compare?")
                                return comparechoiceask
                            if len(prev_selections)>=11:
                                if len(prev_selections)==11 and (not validate_selection(1,2,inputmsg)):
                    ...
                    if smartphone_specs==4:
                        if len(prev_selections)==7:
                            ...
                            for x in google_phones:
                                txt+="{})   {}\n".format(n,x[0])
                                n+=1
                            prev_selections.append("Google phones listed")
                            return txt
                        if len(prev_selections)>=9:
                            if len(prev_selections)==9 and (not validate_selection(1,3,inputmsg)):
                                prev_selections.pop(-1)
                                return "Please enter a valid choice."
                            if len(prev_selections)==9:
                                google_specs=int(inputmsg)
                                compare_lists.append(google_specs-1)
                                comparechoiceask=google_view_or_compare(google_specs)
                                prev_selections.append("View or Compare?")
                                return comparechoiceask
                            if len(prev_selections)>=11:
                                if len(prev_selections)==11 and (not validate_selection(1,2,inputmsg)):
            ...
            if device_specs ==2:
                if len(prev_selections)==5:
                    compare_lists.append(Phone_speclist.copy())
                    prev_selections.append("Tablet manf listed")
                    return """Here's a list of the tablet manufacturers: (Please choose an option)
        1) Apple
        2) Samsung
        """     
                if len(prev_selections)>=7:
                    if len(prev_selections)==7 and (not validate_selection(1,2,inputmsg)):
                        prev_selections.pop(-1)
                        return "Please enter a valid choice."
                    if int(prev_selections[6]) == 1: 
                    # return order_pizza(input, prev_selections)
                    ...
                    if tablet_specs==1:
                        if len(prev_selections)==7:
                            ...
                            for x in Tablets_iphone:
                                txt+="{})   {}\n".format(n,x[0])
                                n+=1
                            prev_selections.append("Ipads listed")
                            return txt
                        if len(prev_selections)>=9:
                            if len(prev_selections)==9 and (not validate_selection(1,4,inputmsg)):
                                prev_selections.pop(-1)
                                return "Please enter a valid choice."
                            if len(prev_selections)==9:
                                ipad_specs=int(inputmsg)
                                compare_lists.append(ipad_specs-1)
                                comparechoiceask=ipad_view_or_compare(ipad_specs)
                                prev_selections.append("View or Compare?")
                                return comparechoiceask
                            if len(prev_selections)>=11:
                                if len(prev_selections)==11 and (not validate_selection(1,2,inputmsg)):
                    ...
                    if tablet_specs==2:
                        if len(prev_selections)==7:
                            ...
                            for x in Tablets_samsung:
                                txt+="{})   {}\n".format(n,x[0])
                                n+=1
                            prev_selections.append("Samsung tablets listed")
                            return txt
                        if len(prev_selections)>=9:
                            if len(prev_selections)==9 and (not validate_selection(1,3,inputmsg)):
                                prev_selections.pop(-1)
                                return "Please enter a valid choice."
                            if len(prev_selections)==9:
                                samsung_tablet_specs=int(inputmsg)
                                compare_lists.append(samsung_tablet_specs-1)
                                comparechoiceask=samsung_tablet_view_or_compare(samsung_tablet_specs)
                                prev_selections.append("View or Compare?")
                                return comparechoiceask
                            if len(prev_selections)>=11:
                                if len(prev_selections)==11 and (not validate_selection(1,2,inputmsg)):
            ...
            if device_specs ==3:
                if len(prev_selections)==5:
                    compare_lists.append(Laptop_speclist.copy())
                    prev_selections.append("Laptop manf listed")
                    return """Here's a list of the laptop manufacturers: (Please choose an option)
        1) Lenovo
        2) Hp
        """     
                    ...
                    if laptop_specs==1:
                        if len(prev_selections)==7:
                            ...
                            for x in laptop_lenovo:
                                txt+="{})   {}\n".format(n,x[0])
                                n+=1
                            prev_selections.append("Lenovo laptops listed")
                            return txt
                        if len(prev_selections)>=9:
                            if len(prev_selections)==9 and (not validate_selection(1,3,inputmsg)):
                                prev_selections.pop(-1)
                                return "Please enter a valid choice."
                            if len(prev_selections)==9:
                                lenovo_laptop_specs=int(inputmsg)
                                compare_lists.append(lenovo_laptop_specs-1)
                                comparechoiceask=lenovo_laptop_view_or_compare(lenovo_laptop_specs)
                                prev_selections.append("View or Compare?")
                                return comparechoiceask
                            if len(prev_selections)>=11:
                                if len(prev_selections)==11 and (not validate_selection(1,2,inputmsg)):
                    ...
                    if laptop_specs==2:
                        if len(prev_selections)==7:
                            ...
                            for x in laptop_HP:
                                txt+="{})   {}\n".format(n,x[0])
                                n+=1
                            prev_selections.append("HP laptops listed")
                            return txt
                        if len(prev_selections)>=9:
                            if len(prev_selections)==9 and (not validate_selection(1,3,inputmsg)):
                                prev_selections.pop(-1)
                                return "Please enter a valid choice."
                            if len(prev_selections)==9:
                                hp_laptop_specs=int(inputmsg)
                                compare_lists.append(hp_laptop_specs-1)
                                comparechoiceask=hp_laptop_view_or_compare(hp_laptop_specs)
                                prev_selections.append("View or Compare?")
                                return comparechoiceask
                            if len(prev_selections)>=11:
                                if len(prev_selections)==11 and (not validate_selection(1,2,inputmsg)):
            ...
            if device_specs==4:
                if len(prev_selections)==5:
                    compare_lists.append(Desktop_speclist.copy())
                    prev_selections.append("PC manf listed")
                    return """Here's a list of the PC manufacturers: (Please choose an option)
        1) Lenovo
        2) Hp
        """
                    ...
                    if pc_specs==1:
                        if len(prev_selections)==7:
                            ...
                            for x in pc_lenovo:
                                txt+="{})   {}\n".format(n,x[0])
                                n+=1
                            prev_selections.append("Lenovo PCs listed")
                            return txt
                        if len(prev_selections)>=9:
                            if len(prev_selections)==9 and (not validate_selection(1,1,inputmsg)):
                                prev_selections.pop(-1)
                                return "Please enter a valid choice."
                            if len(prev_selections)==9:
                                lenovo_pc_specs=int(inputmsg)
                                compare_lists.append(lenovo_pc_specs-1)
                                comparechoiceask=lenovo_pc_view_or_compare(lenovo_pc_specs)
                                prev_selections.append("View or Compare?")
                                return comparechoiceask
                            if len(prev_selections)>=11:
                                if len(prev_selections)==11 and (not validate_selection(1,2,inputmsg)):
                    ...
                    if pc_specs==2:
                        if len(prev_selections)==7:
                            ...
                            for x in pc_hp:
                                txt+="{})   {}\n".format(n,x[0])
                                n+=1
                            prev_selections.append("HP PCs listed")
                            return txt
                        if len(prev_selections)>=9:
                            if len(prev_selections)==9 and (not validate_selection(1,1,inputmsg)):
                                prev_selections.pop(-1)
                                return "Please enter a valid choice."
                            if len(prev_selections)==9:
                                hp_pc_specs=int(inputmsg)
                                compare_lists.append(ice=hp_pc_specs-1)
                                comparechoiceask=hp_pc_view_or_compare(hp_pc_specs)
                                prev_selections.append("View or Compare?")
                                return comparechoiceask
                            if len(prev_selections)>=11:
                                if len(prev_selections)==11 and (not validate_selection(1,2,inputmsg)):
                                ...
                                if int(prev_selections[10]) == 1 and len(prev_selections)==11:
                                    ...
                                    comparechoice=2
            if comparechoice==2:
                if device_specs==1: return second_phone_choice(inputmsg)
                elif device_specs==2:  return second_tablet_choice(inputmsg)
                elif device_specs==3: return second_laptop_choice(inputmsg)
                elif device_specs==4: return second_desktop_choice(inputmsg)
    if cust_need==3:
    ...
    if cust_need==4:
        return locations_function(inputmsg)
    if cust_need==5:
        if len(prev_selections)==53:
            current_application[int(prev_selections[50])-1]= inputmsg
            while len(prev_selections)!=47:
                prev_selections.pop(-1)
        if len(prev_selections) == 3:
            prev_selections.append("Job Applications listed") 
            return """Please choose the desired position: 
1)Delivery Driver
2)Store Clerk
3)Tech Support
4)Office Work
"""     
        if len(prev_selections) == 5:
            if not validate_selection(1,5,inputmsg):
                prev_selections.pop(-1)
                return "Please enter a valid choice."
            prev_selections.append("Enter name:")
            return "Please enter your full name: "
        if len(prev_selections) == 7:
            if not validate_name(inputmsg):
                prev_selections.pop(-1)
                return validation_message[-1]
            current_application.append(inputmsg)#index 0
            current_application.append(jobs[int(prev_selections[4])-1]) #index 1
            prev_selections.append("Enter address:")
            return "Please enter your address: "
        if len(prev_selections) == 9:
            current_application.append(inputmsg) #index 2
            prev_selections.append("Enter P.N.:")
            return "Please enter your phone number: "
        if len(prev_selections)==11:
            if not validate_phonenumber(inputmsg):
                prev_selections.pop(-1)
                return validation_message[-1]
            current_application.append(inputmsg) #index 3
            prev_selections.append("Enter email: ")
            return "Please enter your email address: "
        if len(prev_selections)== 13:
            if not email_validate(inputmsg):
                prev_selections.pop(-1)
                return validation_message[-1]
            current_application.append(inputmsg) #index 4
            prev_selections.append("National ID")
            return "Do you have an Egyption national ID? (Y/N) "
        if len(prev_selections)==15:
            if not validate_YorN(inputmsg):
                prev_selections.pop(-1)
                return validation_message[-1]
            id= inputmsg.upper()
            prev_selections.append("Id no: ")
            if id.upper()=="Y":
                return "Please enter your National Identification Number (Raqam Qawmi): "
            else: 
                current_application.append("None") #index 5
                prev_selections.append("No id")
        if len(prev_selections)==17:
            if prev_selections[14].upper()=="Y":   current_application.append(inputmsg) #index 5 raqam  qawmi
            prev_selections.append("Work schedule?")
            return """Please choose the work schedule your desire:
1) Full-time
2) Part-time
3) Seasonal
""" 
        if len(prev_selections)==19:
            if not validate_selection(1,3,inputmsg):
                prev_selections.pop(-1)
                return "Please select a valid choice: "
            current_application.append(employment_desired[int(prev_selections[18])-1])#index 6
            prev_selections.append("Worked for us? ")
            return "Worked for us before? (Y/N) "
        if len(prev_selections)==21:
            if not validate_YorN(inputmsg):
                prev_selections.pop(-1)
                return validation_message[-1]
            current_application.append(inputmsg.upper()) #index 7
            prev_selections.append("Explain (work)")
            if current_application[7]=="Y": return "Please describe your previous experience including dates: "
            else: 
                current_application.append("None") #index 8
                prev_selections.append("No work history")
        if len(prev_selections)==23:
            if prev_selections[20].upper()=="Y": current_application.append(inputmsg) #index 8
            prev_selections.append("Convicted?")
            return "Have you been convicted of a felony before? (Y/N) "
        if len(prev_selections)==25:
            if not validate_YorN(inputmsg):
                prev_selections.pop(-1)
                return validation_message[-1]
            current_application.append(inputmsg.upper()) #index 9
            prev_selections.append("Explain (Felony)")
            if current_application[9]=="Y": return "Please explain: "
            else: 
                current_application.append("None") #index 10
                prev_selections.append("No felonies")
        if len(prev_selections)==27:
            if prev_selections[24].upper()=="Y": current_application.append(inputmsg) #index 10
            prev_selections.append("HS name: ")
            return "Please enter your High School's Name, including city: "
        if len(prev_selections)==29:
            current_application.append(inputmsg) #index 11
            prev_selections.append("HS Start: ")
            return "Please enter your High School start date (DD/MM/YYYY): "
        if len(prev_selections)==31:
            if not validate_date(inputmsg):
                prev_selections.pop(-1)
                return validation_message[-1]
            current_application.append(inputmsg) #index 12
            prev_selections.append("HS End: ")
            return "Please enter your High School end date (DD/MM/YYYY): "
        if len(prev_selections)==33:
            if not validate_date(inputmsg):
                prev_selections.pop(-1)
                return validation_message[-1]
            current_application.append(inputmsg) #index 13
            prev_selections.append("HS Grad?")
            return "Did you graduate High School? (Y/N) "
        if len(prev_selections)==35:
            if not validate_YorN(inputmsg):
                prev_selections.pop(-1)
                return validation_message[-1]
            current_application.append(inputmsg.upper()) #index 14
            prev_selections.append("College?")
            return "Did you go to college? (Y/N) "
        if len(prev_selections)==37:
            if not validate_YorN(inputmsg):
                prev_selections.pop(-1)
                return validation_message[-1]
            if prev_selections[36].upper()=="Y":
                prev_selections.append("College Name")
                return "Please enter your College's Name, including city: "
            else: 
                ...
                while len(prev_selections)!=47:
                    prev_selections.append("None")
        if len(prev_selections)==39:
            current_application.append(inputmsg) #index 15
            prev_selections.append("College start")
            return "Please enter your College start date (DD/MM/YYYY): "
        if len(prev_selections)==41:
            if not validate_date(inputmsg):
                prev_selections.pop(-1)
                return validation_message[-1]
            current_application.append(inputmsg) #index 16
            prev_selections.append("College end")
            return "Please enter your College end date (DD/MM/YYYY): "
        if len(prev_selections)==43:
            if not validate_date(inputmsg):
                prev_selections.pop(-1)
                return validation_message[-1]
            current_application.append(inputmsg) #index 17
            prev_selections.append("College grad?")
            return "Did you graduate College? (Y/N) "
        if len(prev_selections)==45:
            if not validate_YorN(inputmsg):
                prev_selections.pop(-1)
                return validation_message[-1]
            current_application.append(inputmsg.upper()) #index 18
            prev_selections.append("Degree?")
            if current_application[18]=="Y": return "Please enter your Degree: "
            else:
                current_application.append("None") #index 19
                prev_selections.append("No degree")
        if len(prev_selections)==47:
            if len(current_application)<20 and prev_selections[44].upper()=="Y": current_application.append(inputmsg) #index 19
            prev_selections.append("Data correct? ")
            application="\n"
            for x in range(len(application_info)):
                application+="{}) {}:   {}\n".format(x+1,application_info[x],current_application[x])
            application+="Is all your data correct? (Y/N) "
            return application
        if len(prev_selections)==49:
            if not validate_YorN(inputmsg):
                prev_selections.pop(-1)
                return validation_message[-1]
            if inputmsg.upper()=="N":
                prev_selections.append("Wrong data no.")
                return "Please enter the number of the wrong data: "
            else: 
                dir= os.path.dirname(__file__)
                dir=dir.replace("\\","/")
                ...
                return """Thank you {} for your application to the {} role at Azka-tronics.
    ...
    """.format(current_application[0],current_application[1],current_application[3])
        if len(prev_selections)==51:
            if not validate_selection(1,20,inputmsg):
                prev_selections.pop(-1)
                return "Please choose a valid choice: "
            prev_selections.append("Correct data")
            return "Please enter the corrected data: "
